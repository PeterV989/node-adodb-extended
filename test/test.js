...
if (fs.existsSync(cscript) && fs.existsSync(source)) {
  ...
  describe('ADODB', () => {
    ...
    it('query_v2 (Array)', next => {
      connection
        .query_v2('SELECT * FROM Users', true)
        .then(data => {
          expect(Object.keys(data).length).to.equal(2);
          expect(data).to.have.ownProperty('type');
          expect(data.type).to.equal(true);
          expect(data.ResultSet.FieldMetaData).to.have.ownProperty('FieldNames');
          expect(data.ResultSet.FieldMetaData.FieldNames.length).to.equal(5);
          expect(data.ResultSet.ResultSet[0][1]).to.equal('Nuintun');
          expect(data.ResultSet.FieldMetaData.FieldNames[1]).to.equal('UserName');
          expect(data.ResultSet.ResultSet[2][1]).to.equal('张三');
          next();
        })
        .catch(next);
    })

    it('query_v2 (Records)', next => {
      connection
        .query_v2('SELECT * FROM Users', false)
        .then(data => {
          expect(Object.keys(data).length).to.equal(2);
          expect(data).to.have.ownProperty('type');
          expect(data.type).to.equal(false);
          expect(data.ResultSet.length).to.equal(3);
          expect(data.ResultSet[0]).to.have.ownProperty('UserName');
          expect(data.ResultSet[0].UserName).to.equal('Nuintun');
          expect(data.ResultSet[2].UserName).to.equal('张三');
          next();
        })
        .catch(next);
    })
